...
actor {
  ...
  // Self-register a local user — only creates if username doesn't already exist
  // Used to sync users.json entries into the backend so admin operations work
  public func selfRegisterLocalUser(username : Text, password : Text, name : Text, accessLevel : Text) : async { #ok : Nat; #alreadyExists } {
