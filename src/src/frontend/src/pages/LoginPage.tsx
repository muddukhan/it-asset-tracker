...
import {
  ...
  findInLocalUserRegistry,
} from "../utils/localUserRegistry";
...
export function LoginPage({
  ...
  // Helper: sync a user to the backend (best-effort, non-blocking)
  ...
  const handleLocalLogin = async () => {
    ...
    try {
      ...
      // backend failed — fall through to fallbacks
    }

    // 2. localStorage registry fallback (users created via admin panel)
    const registryMatch = findInLocalUserRegistry(username.trim(), password);
    if (registryMatch) {
      const session: LocalSession = {
        name: registryMatch.name,
        accessLevel: registryMatch.accessLevel,
        username: registryMatch.userId,
        password: password,
      };
      localStorage.setItem("localUserSession", JSON.stringify(session));
      // Sync to backend so admin operations work
      syncUserToBackend(
        registryMatch.userId,
        password,
        registryMatch.name,
        registryMatch.accessLevel,
      );
      onLocalLogin(session);
      return;
    }

    // 3. Static users.json fallback
    try {
